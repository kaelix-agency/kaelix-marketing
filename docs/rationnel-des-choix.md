...
### 1.16 Pourquoi la revue hebdo est strictement interne et le rapport mensuel le seul artefact client
Décision opérateur du 2026-08-28 (transports-ansquer, première revue hebdo). Un « pulse client » hebdo (3-4 lignes WhatsApp en annexe de la revue) a été essayé le jour même puis **retiré** : il mélangeait deux régimes de communication et créait un deuxième canal à valider, à tracer et à figer — pour un contenu que l'opérateur porte mieux en direct. Doctrine retenue : **la `/weekly-review` est uniquement interne** (pilotage opérateur + repo : constats, règle des 90 j, plan d'action, statuts du plan) et ne produit aucun message client. **La communication client = le rapport mensuel (`/report`), seul artefact à destination du client** — un livrable fini, en PDF, envoyable tel quel (§1.17). **Les demandes ponctuelles (first-party, avis, validations) passent par l'opérateur en direct, hors système** : la revue les liste comme actions avec le canal « WhatsApp opérateur », elle ne les rédige pas. Conséquence : un seul gate de communication client par mois, une seule trace figée, et le canal direct reste humain.

### 1.17 Pourquoi le rapport mensuel est un PDF « prêt à l'envoi » généré par script
Demande opérateur du 2026-08-28 : le `/report` produisait un markdown interne, inenvoyable sans remise en forme — donc jamais envoyé tel quel, donc retouché à la main, donc non figé. Le livrable client doit être **fini** : un PDF sobre (en-tête KAELIX, identité légère, tableaux propres, 2-3 pages), généré depuis le markdown du rapport par `scripts/generate-report-pdf.mjs` (Node + `marked` + Playwright/Chromium, déjà présents sur la machine ; aucun outil externe type pandoc/LaTeX). Structure client imposée, zéro jargon : ① ce qui a été fait ce mois · ② ce que ça donne (chiffres **si consolidés**, sinon la phrase honnête « période de fondations, premières données au mois 2-3 » — jamais de métrique gonflée, invariant 9) · ③ ce qui arrive le mois prochain · ④ ce dont on a besoin de votre côté. Circuit : `/report` génère le markdown (🕓 brouillon, bandeau interne avec les points « à confirmer ») → gate opérateur (✅ validé) → génération du PDF → 📤 émis à l'envoi par l'opérateur ; markdown et PDF sont alors figés ensemble. Le bandeau interne n'est jamais rendu dans le PDF (marqueur `<!-- CLIENT -->`) ; un rapport non validé porte un bandeau « BROUILLON » dans le PDF pour qu'un brouillon ne parte jamais par erreur. **Format validé et figé au gate du 2026-08-28** (gabarit transports-ansquer, août 2026) comme **standard des rapports clients KAELIX** : structure ①-④, tableau « Réalisation / Date / Où le voir » cliquable, section ② au ton honnête (« un signal, pas un résultat »), bandeau brouillon, pied paginé. Le template (`clients/_template/reports/rapport-client.template.md`) et la feuille de style du script en sont la référence ; on ne redessine pas la mise en page par client.

### 1.18 Pourquoi une to-do opérateur par client (`todo-operateur.md`)
Décision opérateur du 2026-08-28. Le système trace ce qu'il fait (tracking, plan, revues, rapports) mais les actions qui reviennent à **l'humain** (contacter le client, valider, arbitrer, donner un accès, envoyer) étaient dispersées : dans le plan d'action d'une revue, dans la section ④ d'un rapport, dans le journal d'un brief, dans la conversation. Résultat : des actions perdues entre deux sessions et des productions bloquées à J-7 faute de first-party demandé à temps. Convention : `clients/<slug>/todo-operateur.md`, un tableau [action | origine | échéance | statut], **maintenu à chaque `/weekly-review` (étape 11) et à chaque gate**, alimenté par la section ④ du `/report`. C'est la liste de l'opérateur, pas du système : elle ne contient aucune action que la session peut exécuter elle-même. Ce qui conditionne une production porte la date-limite J-7 du sujet, pour que le glissement soit une décision, jamais une surprise. Gabarit dans `clients/_template/`.
