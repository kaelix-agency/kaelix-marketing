...
| 08/2026 | création | Empotage et dépotage de conteneur : guide complet du chargeur (étapes, coûts, responsabilités, code CTU) — sans « Gennevilliers » dans le title | empotage conteneur (140) + depotage conteneur (70) — fusion, SERPs recouvrantes | info-commerciale | P2 | guide | A | devis `/devis/` | implantation dans le port (G1, quai 8) ; 🟡 partiel 2026-08-06 : durée « dépend du contenu » + anecdote renfort main-d'œuvre (⚠️ chiffres à reconfirmer) ; grille ⛔ refusée ; photos : stock (décision 2026-08-24) | **en PR** — GO opérateur 2026-08-28 (brief `research/2026-08-28-guide-empotage-depotage-conteneur.md`), `/write` le jour même : double PASS (fact-checker 30 points, 4 🟠 corrigés + re-contrôle ; contract-checker 17 critères + re-contrôle), **PR #5 du repo site ouverte**, build vert ; 2 schémas SVG first-party + cover stock ; merge = gate opérateur (preview Vercel) |
