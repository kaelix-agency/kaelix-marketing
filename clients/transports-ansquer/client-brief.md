...
|---|---|---|
| Search Console (**API directe**, `scripts/gsc-fetch.mjs` — remplace Cuik le 2026-09-03, rationnel §1.24) | ✅ **CONNECTÉE le 2026-09-03** : compte de service `gsc-fetch@kaelix-498715.iam.gserviceaccount.com` (projet GCP existant `kaelix-498715`, clé hors repo `~\.kaelix\`), ajouté « Complet » sur la propriété | ⚠️ **La propriété vérifiée est le PRÉFIXE D'URL `https://transportsansquer.fr/`** (pas une propriété Domaine — constat du 03/09, défaut du script aligné). **Baseline 28 j (05/08-01/09)** : 20 clics / 668 impressions, homepage pos. 15,2 (16 clics) ; art. tournée : 19 imprs pos. 9,2, **indexé** (crawl 26/08) ; « course urgente » 75 imprs pos. 22,8, « crossdocking île de france » 42 imprs pos. 4,2. **Sitemap courant soumis le 03/09** (24 URLs — seul l'ancien `page-sitemap.xml` WordPress de 2022 l'était, 1 erreur, 0 indexé : à retirer de GSC ?) ; `check404` ✅ (24 sitemap + 19 anciennes URLs, aucune anomalie). Articles 2-3 inconnus de Google (J+6/J+1, normal) : demande d'indexation par bouton GSC (l'Indexing API exige la propriété déléguée : 403) |
