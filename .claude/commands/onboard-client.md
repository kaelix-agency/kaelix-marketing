...
1. Crée `clients/<slug>/` en copiant les gabarits de `clients/_template/` (inclure `nap.md` seulement si type local/ecommerce avec point de vente).
...
5. **Baseline technique (Cuik MCP)** : crawl initial du domaine + état Search Console si connecté. Résume les 5 problèmes les plus importants. Vérifier notamment que le contenu est servi en HTML complet (SSG/SSR) — les IA lisent mal le rendu JavaScript client.
...
8. Termine par un plan des 5 premiers contenus/actions, priorisés selon la méthode (l'existant 4-20 passe souvent AVANT la création).
