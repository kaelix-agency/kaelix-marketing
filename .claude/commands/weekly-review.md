...
12. **Circuit avis (client local — rationnel §1.25)** : relève les avis de la **fiche publique** (fetch de l'URL Maps du place_id / lien de la fiche — pas d'API GBP) et compare au registre `clients/<slug>/avis.md`. **Chaque nouvel avis** : le signaler dans l'essentiel en 5 lignes de la revue, générer le brouillon via `/review-response` (qui trace dans `avis.md` **et crée l'issue GitHub de notification** — titre `⭐ Nouvel avis <note>/5 …`, 🔴 URGENT si ≤3 étoiles). ⛔ Jamais de publication automatique. Fiche injoignable → « ⚠️ contrôle empêché » avec raison.
13. **Citations (client local, mensuel — 1er lundi du mois seulement)** : contrôle léger de `clients/<slug>/citations.md` — re-fetch des fiches au statut `soumis`/`vérifié` (budget 2 essais/annuaire), mise à jour des colonnes état/écart/statut, écarts nouveaux signalés dans la revue. Les autres lundis : sauter cette étape.

> ⛔ **Périmètre** (décisions opérateur 2026-08-28 et 2026-09-02, `docs/rationnel-des-choix.md` §1.16 et §1.20, invariant 14) : la revue hebdo est **uniquement interne** — pilotage opérateur + repo. Elle ne produit **aucun message à destination du client**. **Les actions « demander au client X » sont INTERDITES** dans le plan d'action : un manque de first-party se traite par **« ré-angler / glisser / faire sans »** (règle de dégradation du plan), jamais par une sollicitation. Le seul canal de demande est la ligne avis permanente du rapport mensuel.
