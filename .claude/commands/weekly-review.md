...
11. **Pulse client** (décision opérateur 2026-08-28 — `docs/rationnel-des-choix.md` §1.16) : rédige en annexe de la revue un message de **3-4 lignes en langage client, zéro jargon SEO**, prêt à coller dans le canal du client (WhatsApp par défaut, cf. brief §1). Contenu : ce qui a été mis en ligne cette semaine (liens), les informations first-party dont la production a besoin (questions concrètes, courtes), et une relance douce si une action client est en attente (avis, validation). **Interdits** : chiffres de trafic non consolidés, positions, promesses de résultat, tout ce qui relève du rapport mensuel. Statut 🕓 brouillon → ✅ validé (gate opérateur) → 📤 envoyé le JJ/MM ; l'opérateur envoie, jamais la session. Si aucune nouveauté et aucun besoin : pas de pulse cette semaine (le signaler, ne pas meubler).
