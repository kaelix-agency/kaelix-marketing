...
11. **Contrat de contenu.** Toute production est du **MDX sémantique pur** conforme à `docs/contrat-de-contenu.md` : frontmatter valide (schéma typé, validé au build du site), zéro style/HTML de mise en page dans le corps, composants limités à la whitelist (`Callout`, `CTA`, `FAQ`). Le design appartient aux sites (conteneur typographique + mapping MDX) — ne jamais « corriger » l'apparence depuis le contenu.
